...
import { Search, Send, Phone, Video, MoreVertical, ArrowLeft, Image as ImageIcon, Smile } from 'lucide-react';
...
const MessagesPage: React.FC = () => {
  ...
  useEffect(() => {
    if (activeConversation) {
      // TODO: Replace with API.get(`/messages/${activeConversation.id}`)
      setMessages([
        { id: 'm1', senderId: 'other', text: 'Hey there!', timestamp: '10:30 AM' },
        { id: 'm2', senderId: CURRENT_USER_ID, text: 'Hi Alex! How are you?', timestamp: '10:32 AM' },
        { id: 'm3', senderId: 'other', text: 'Doing well. Are we still on for tomorrow?', timestamp: '10:42 AM' },
      ]);
      scrollToBottom();
    }
  }, [activeConversation]);
  ...
  return (
    ...
    <div className="flex h-[calc(100vh-4rem)] bg-white overflow-hidden border-t border-gray-200">
      ...
      <div className={`w-full md:w-1/3 lg:w-1/4 border-r border-gray-200 flex flex-col ${isMobileChatOpen ? 'hidden md:flex' : 'flex'}`}>
          ...
          {conversations.map((conv) => (
            ...
            >
              <div className="relative flex-shrink-0">
                <img src={conv.avatarUrl} alt={conv.participantName} className="h-12 w-12 rounded-full object-cover" />
                {conv.isOnline && (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-baseline mb-1">
                  <h3 className="text-sm font-semibold text-gray-900 truncate">{conv.participantName}</h3>
      ...
      <div className={`w-full md:w-2/3 lg:w-3/4 flex flex-col bg-gray-50/50 ${!isMobileChatOpen ? 'hidden md:flex' : 'flex'}`}>
        ...
        {activeConversation ? (
          <>
            ...
            <div className="flex items-center justify-between p-4 bg-white border-b border-gray-200">
              <div className="flex items-center gap-3">
                ...
                <img src={activeConversation.avatarUrl} alt={activeConversation.participantName} className="h-10 w-10 rounded-full object-cover" />
                <div>
                  <h3 className="text-md font-semibold text-gray-900">{activeConversation.participantName}</h3>
                  <p className="text-xs text-gray-500">{activeConversation.isOnline ? 'Active now' : 'Offline'}</p>
                </div>
              </div>
              <div className="flex items-center gap-2 text-gray-500">
                <button className="p-2 hover:bg-gray-100 rounded-full transition"><Phone className="h-5 w-5" /></button>
                <button className="p-2 hover:bg-gray-100 rounded-full transition"><Video className="h-5 w-5" /></button>
                <button className="p-2 hover:bg-gray-100 rounded-full transition"><MoreVertical className="h-5 w-5" /></button>
              </div>
            </div>
            ...
            <div className="p-4 bg-white border-t border-gray-200">
              <form onSubmit={handleSendMessage} className="flex items-end gap-2">
                <div className="flex items-center gap-2 text-gray-400 pb-2">
                  <button type="button" className="p-2 hover:text-blue-600 hover:bg-gray-100 rounded-full transition"><ImageIcon className="h-5 w-5" /></button>
                  <button type="button" className="p-2 hover:text-blue-600 hover:bg-gray-100 rounded-full transition hidden sm:block"><Smile className="h-5 w-5" /></button>
                </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
            ...
            <p className="text-gray-500 max-w-sm">Select a conversation from the sidebar to start chatting or start a new conversation.</p>
          </div>
        )}
      </div>
